...
/* Buffers DMA en SRAM D2 (0x30000000) - accesible por DMA1 */
__attribute__((section(".RAM_D2_bss")))
uint32_t dma_buf_a[AUDIO_DMA_BUFFER_SIZE];

__attribute__((section(".RAM_D2_bss")))
uint32_t dma_buf_b[AUDIO_DMA_BUFFER_SIZE];/* Global audio context */
...
void audio_capture_deinterleave(AudioCaptureContext *ctx, uint8_t half)
{
    ...
    for (uint32_t i = 0; i < AUDIO_BUFFER_SIZE; i++)
    {
        ...
        ctx->ch0[offset + i] = (int32_t)dma_buf_a[stereo_idx];
        
        /* Mic2 (Right channel from SAI_A) */
        ctx->ch1[offset + i] = (int32_t)dma_buf_a[stereo_idx + 1];
    }
    ...
    for (uint32_t i = 0; i < AUDIO_BUFFER_SIZE; i++)
    {
        ...
        ctx->ch2[offset + i] = (int32_t)dma_buf_b[stereo_idx];
        
        /* Mic4 (Right channel from SAI_B) */
        ctx->ch3[offset + i] = (int32_t)dma_buf_b[stereo_idx + 1];
    }
}
