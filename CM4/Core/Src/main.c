...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  {
      flash_log("CM4: ERROR audio_capture_init\n");
      /* Parpadeo azul rapido = fallo audio_capture_init */
      while(1) {
          HAL_GPIO_TogglePin(GPIOK, GPIO_PIN_7);
          HAL_Delay(200);
      }
  }
  flash_log("CM4: audio_capture_init OK\n");

  if (audio_capture_start(&g_audio_ctx) != 0)
  {
      flash_log("CM4: ERROR audio_capture_start\n");
      /* Parpadeo rojo rapido = fallo audio_capture_start */
      while(1) {
          HAL_GPIO_TogglePin(GPIOK, GPIO_PIN_5);
          HAL_Delay(200);
      }
  }
  flash_log("CM4: audio_capture_start OK\n");
